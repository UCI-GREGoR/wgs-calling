...
def construct_nonexport_files(wildcards, manifest: pd.DataFrame, suffix: str) -> list:
    """
    Use checkpoint output of linker generation rule to
    determine what files need to be constructed for a
...
rule create_nonexported_manifest:
    ...
    """
    input:
        vcf=lambda wildcards: construct_nonexport_files(wildcards, manifest, "snv.vcf.gz"),
        tbi=lambda wildcards: construct_nonexport_files(wildcards, manifest, "snv.vcf.gz.tbi"),
        vcf_md5=lambda wildcards: construct_nonexport_files(wildcards, manifest, "snv.vcf.gz.md5"),
        tbi_md5=lambda wildcards: construct_nonexport_files(
            wildcards, manifest, "snv.vcf.gz.tbi.md5"
        ),
    output:
